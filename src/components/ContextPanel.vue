...
const parentServer = computed(() => {
  ...
  if (selectedPart.value) {
    return gameStore.servers.find(s => s.installedParts.some(p => p.id === selectedPart.value!.id)) || null;
  }
  return null;
});
...
<template>
  <div style="flex: 1; min-width: 350px; display: flex; flex-direction: column; gap: 20px;">
    ...
    <!-- SERVER NODE PANEL -->
    <div v-if="selectedServer" style="border: 2px solid black; padding: 15px; background: white; border-radius: 0;">
      ...
      <!-- Active Job Progress -->
      <div v-if="isRunningJob && activeJob" style="margin: 20px 0; border: 2px solid green; padding: 10px; background: #e0ffe0;">
    ...
    <!-- PART INFO PANEL -->
    <div v-if="displayedPart" style="border: 2px solid black; padding: 15px; background: white; border-radius: 0;">
      <h3 style="margin-top: 0; display: flex; justify-content: space-between; align-items: flex-start;">
      ...
      <!-- SLOTS -->
      <div v-if="displayedPart.slots && displayedPart.slots.length > 0 && parentServer" style="margin-top: 15px; border-top: 2px solid #ccc; padding-top: 10px;">
