...
import { ROOM_WIDTH, ROOM_HEIGHT, isEmptySpace, isValidPlacement, isSupportingAnotherObject, type RoomRect } from '../utils/physics';
...
export function useDraggable(roomItems: Ref<RoomRect[]>, config: DragConfig) {
  ...
  const isDragValid = computed(() => {
    ...
    if (!isEmptySpace(finalX, finalY, item.width, item.height, item.id, roomItems.value)) return false;
    
    let fallY = finalY;
    while (fallY <= ROOM_HEIGHT - item.height && isEmptySpace(finalX, fallY, item.width, item.height, item.id, roomItems.value)) {
      fallY++;
    }
    fallY--;
